...
lineage_shortlist = function(df, cut = FALSE) {
  
  if(cut){
    df_filt <- df |> 
      mutate(ShortLin = case_when(startsWith(pango_lineage, "X") ~ substr(pango_lineage, 1, 5),
                                  TRUE ~ substr(pango_lineage, 1, 4)))
  ...
  df_filt = df %>%
    mutate(ShortLin = case_when(
      ...
      startsWith(ShortLin, "EG") ~ "EG",
      startsWith(ShortLin, "FL") ~ "FL",
      startsWith(ShortLin, "FU") ~ "FU",
      TRUE ~ "Other"
    )) 
}
